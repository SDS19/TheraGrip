...
import Handeinheit_10_03 as hand
import os
import canopen
...
#Operation Enable Controlword and check the following Statusword. Checking several Statuswords because of various options. look at Bit assignment Statusword, data package in user manual
    ...
    def profile_velocity_mode(self, velo_list):
        self.setMode(3)
        self.sendCommand(self.enableOperation_array)     
        # acceleration = 1000
        #self.sendCommand(bytearray([0, 0, 0, 0, 0, 17, 1, 43, 13, 1, 0, 0, 96, 131, 0, 0, 0, 0, 4, 160, 134, 1, 0]))
        self.sendCommand(bytearray([0, 0, 0, 0, 0, 17, 1, 43, 13, 1, 0, 0, 96, 131, 0, 0, 0, 0, 4, velo_list[4], velo_list[5], velo_list[6], velo_list[7]]))
        self.sendCommand(bytearray([0, 0, 0, 0, 0, 17, 1, 43, 13, 1, 0, 0, 96, 132, 0, 0, 0, 0, 4, velo_list[4], velo_list[5], velo_list[6], velo_list[7]]))
        self.sendCommand(bytearray([0, 0, 0, 0, 0, 17, 1, 43, 13, 1, 0, 0, 96, 255, 0, 0, 0, 0, 4, velo_list[0], velo_list[1], velo_list[2], velo_list[3]]))
...
def get_list(n, start_point, end_point):
    ...
    if d_x != 0 and d_y != 0:
        ...
        t_x_list = get_time_list(d_x, v_x_list)
        t_y_list = get_time_list(d_y, v_y_list)
...
def move(start_point, end_point, velo_acc_x_mat, velo_acc_y_mat, time_list):
    start_x = start_point[0]
    start_y = start_point[1]
    end_x = end_point[0]
    end_y = end_point[1]
    ...
    (v_x_list, v_y_list, v_list) = get_xy_velocity_list(sample_num, abs(d_x), abs(d_y))
    print(v_x_list)
    
    t_x_list = get_time_list(abs(d_x), v_x_list)
    t_y_list = get_time_list(abs(d_y), v_y_list)
    ...
    if d_x != 0.0 and d_y != 0.0:
        for i in range(1, sample_num + 1):
            ...
            X_Motor.profile_velocity_mode(velo_acc_x_mat[i])
            Y_Motor.profile_velocity_mode(velo_acc_y_mat[i])
            ...
            print("interpolation: " + str(X_Motor.get_actual_position()) + ", " + str(Y_Motor.get_actual_position()))
...
def update_data(i):
    real_x_velo_array[i] = X_Motor.get_actual_velocity()
    real_y_velo_array[i] = Y_Motor.get_actual_velocity()
    real_x_posi_array[i] = X_Motor.get_actual_position()
    real_y_posi_array[i] = Y_Motor.get_actual_position()
...
# mp2_x = mp0_x - 0.0
# mp2_y = mp0_y - 200.0
# mp3_x = mp0_x - m.ceil(125.0 * m.cos(m.radians(45)))
# mp3_y = mp0_y - m.ceil(125.0 * m.sin(m.radians(45)))
# mp4_x = mp0_x - m.ceil(shoulder_width*0.5)
# mp4_y = mp0_y - m.ceil(shoulder_width*0.5)
# mp5_x = mp0_x - 125.0
# mp5_y = mp0_y - 0.0
# mp6_x = mp0_x - m.ceil(shoulder_width)
# mp6_y = mp0_y - m.ceil(shoulder_width)

# (mp1_x_velo_list, mp1_y_velo_list, mp1_c_velo_list, mp1_x_velo_acc_mat, mp1_y_velo_acc_mat, mp1_op_time_n_list) = get_list(sample_num, mp0_x, mp1_x, mp0_y, mp1_y)
# (mp2_x_velo_list, mp2_y_velo_list, mp2_c_velo_list, mp2_x_velo_acc_mat, mp2_y_velo_acc_mat, mp2_op_time_n_list) = get_list(sample_num, mp0_x, mp2_x, mp0_y, mp2_y)
# (mp3_x_velo_list, mp3_y_velo_list, mp3_c_velo_list, mp3_x_velo_acc_mat, mp3_y_velo_acc_mat, mp3_op_time_n_list) = get_list(sample_num, mp0_x, mp3_x, mp0_y, mp3_y)
# (mp4_x_velo_list, mp4_y_velo_list, mp4_c_velo_list, mp4_x_velo_acc_mat, mp4_y_velo_acc_mat, mp4_op_time_n_list) = get_list(sample_num, mp0_x, mp4_x, mp0_y, mp4_y, forward_x, forward_y)
# (mp5_x_velo_list, mp5_y_velo_list, mp5_c_velo_list, mp5_x_velo_acc_mat, mp5_y_velo_acc_mat, mp5_op_time_n_list) = get_list(sample_num, mp0_x, mp5_x, mp0_y, mp5_y, forward_x, forward_y)
# (mp6_x_velo_list, mp6_y_velo_list, mp6_c_velo_list, mp6_x_velo_acc_mat, mp6_y_velo_acc_mat, mp6_op_time_n_list) = get_list(sample_num, mp0_x, mp6_x, mp0_y, mp6_y, forward_x, forward_y)
...
# def setMoveHand():
#     node_1.setprofile_position_mode()
#     node_2.setprofile_position_mode()
...
# def moveHand():
#     
#     node_1.profile_position_mode(node_1.end_position)
#     node_2.profile_position_mode(node_2.end_position)
#     
#     while(node_1.getget_actual_velocity()!=0 or node_2.getget_actual_velocity()!=0):
#         time.sleep(0.1)
#     
#     print("1 reached:"+str(node_1.node.sdo[0x6041].bits[10]))
#     print("2 reached:"+str(node_2.node.sdo[0x6041].bits[10]))
#     
#     print("Position 1: "+str(node_1.getget_actual_position()))
#     print("Position 2: "+str(node_2.getget_actual_position()))
#     print('\n')
#     
#     node_1.switchOn()
#     node_2.switchOn()
#     
#     node_1.profile_position_mode(node_1.start_position)
#     node_2.profile_position_mode(node_2.start_position)
#     
#     while(node_1.getget_actual_velocity()!=0 or node_2.getget_actual_velocity()!=0):
#         time.sleep(0.1)
#     
...
#                   move(d_x, d_y, velo_acc_x_mat,velo_acc_y_mat,op_time_n_list)
                    move([start_x, start_y], [target_x, target_y], velo_acc_x_mat,velo_acc_y_mat,op_time_n_list)
