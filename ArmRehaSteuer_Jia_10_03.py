...
def veloCurve(x):
    y = (-5.83397691012945e-14*m.pow(x,9)+4.01458396693356e-11*m.pow(x,8)-9.6248203709691e-9*m.pow(x,7)+\
        ...
        1.24822800434351e-2*m.pow(x,3)+0.531322885004075*m.pow(x,2)-0.240497493514033*x+0.234808880863676)*0.5
    return y
...
def calcAccList(t_list, v_list):
    ...
    '''
    if sample_num == 20:
        acc_list[7] = m.floor(acc_list[7]*0.6)
        acc_list[8] = m.floor(acc_list[8]*0.6)
        acc_list[9] = m.floor(acc_list[9]*0.6)
        
    if sample_num == 15:
        acc_list[4] = m.floor(acc_list[4]*0.6)
        acc_list[5] = m.floor(acc_list[5]*0.6)  
    ...
    if sample_num == 5:
        acc_list[2] = m.floor(acc_list[2]*0.6)
    '''
...
def posNumToFourByte(num):
    ...
    if num >= 16777216 and num < 4294967296:
        ...
        byte_list[0] = int(num - ((byte_list[3]*m.pow(256, 3))+(byte_list[2]*m.pow(256, 2))+ (byte_list[1]*256)))
...
def move(dist_x, dist_y, velo_acc_x_mat, velo_acc_y_mat, time_list):
...
def plot(dist_x, dist_y, start_x, start_y):
    ...
    plt.title("Interpolation Points: "+str(sample_num)+"\nDistance X: "+str(dist_x)+"mm, Distance Y: "+str(dist_y)+"mm")
...
def loosenMode(target_x_list, target_y_list, frequency, v_x_list, v_y_list, position_a):
    ...
    X_Motor.ProfPosiMode(100, 150, lm_target_x_list[0])
    Y_Motor.ProfPosiMode(100, 150, lm_target_y_list[0])
    while((X_Motor.actualVelocity() != 0) or (Y_Motor.actualVelocity() != 0)):
        time.sleep(0.2)
...
(velo_x_list, velo_y_list, velo_c_list, velo_acc_x_mat, velo_acc_y_mat, op_time_n_list) = calcVeloAcc(sample_num, start_x, target_x, start_y, target_y, forward_x, forward_y)
...
(mp1_x_velo_list, mp1_y_velo_list, mp1_c_velo_list, mp1_x_velo_acc_mat, mp1_y_velo_acc_mat, mp1_op_time_n_list) = calcVeloAcc(sample_num, mp0_x, mp1_x, mp0_y, mp1_y, forward_x, forward_y)
(mp2_x_velo_list, mp2_y_velo_list, mp2_c_velo_list, mp2_x_velo_acc_mat, mp2_y_velo_acc_mat, mp2_op_time_n_list) = calcVeloAcc(sample_num, mp0_x, mp2_x, mp0_y, mp2_y, forward_x, forward_y)
(mp3_x_velo_list, mp3_y_velo_list, mp3_c_velo_list, mp3_x_velo_acc_mat, mp3_y_velo_acc_mat, mp3_op_time_n_list) = calcVeloAcc(sample_num, mp0_x, mp3_x, mp0_y, mp3_y, forward_x, forward_y)
(mp4_x_velo_list, mp4_y_velo_list, mp4_c_velo_list, mp4_x_velo_acc_mat, mp4_y_velo_acc_mat, mp4_op_time_n_list) = calcVeloAcc(sample_num, mp0_x, mp4_x, mp0_y, mp4_y, forward_x, forward_y)
(mp5_x_velo_list, mp5_y_velo_list, mp5_c_velo_list, mp5_x_velo_acc_mat, mp5_y_velo_acc_mat, mp5_op_time_n_list) = calcVeloAcc(sample_num, mp0_x, mp5_x, mp0_y, mp5_y, forward_x, forward_y)
(mp6_x_velo_list, mp6_y_velo_list, mp6_c_velo_list, mp6_x_velo_acc_mat, mp6_y_velo_acc_mat, mp6_op_time_n_list) = calcVeloAcc(sample_num, mp0_x, mp6_x, mp0_y, mp6_y, forward_x, forward_y)
            ...
            if values["control_point"] != '':
                ...
                if values["control_point"]== 1 and event in (None, 'CP_Start') :
                    ...
                    if target_x <= 250.0 and target_x >= 0.0 and target_y <= 250.0 and target_y >= 0.0:
                        ...
                        move(abs(mp1_x-mp0_x), abs(mp1_y-mp0_y), mp1_x_velo_acc_mat, mp1_y_velo_acc_mat, mp1_op_time_n_list)
                ...
                if values["control_point"]== 2 and event in (None, 'CP_Start') :
                    ...
                    if target_x <= 250.0 and target_x >= 0.0 and target_y <= 250.0 and target_y >= 0.0:
                        ...
                        move(mp2_x-mp0_x, mp2_y-mp0_y, mp2_x_velo_acc_mat, mp2_y_velo_acc_mat, mp2_op_time_n_list)
